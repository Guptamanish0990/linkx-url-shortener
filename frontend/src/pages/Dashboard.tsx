...
export default function Dashboard() {
  const { token, user }: { token: string | null; user: User | null } = useAuth();
  const [urls, setUrls] = useState<Url[]>([]);
  const [error, setError] = useState<string | null>(null);
  const [stats, setStats] = useState<Stats>({ totalClicks: 0 });
  ...
  useEffect(() => {
    ...
    };  // ✅ YE LINE ADD KI — loadData function properly close ho raha hai ab

    loadData();
  }, [token]);
  // Log current state for debugging
  useEffect(() => {
    console.log('Dashboard State - URLs:', urls, 'Stats:', stats, 'Error:', error, 'Loading:', loading);
  }, [urls, stats, error, loading]);
  // ============================================================
  ...
  const handleShorten = async (e: FormEvent) => {
    ...
    try {
      const payload: { url: string, customAlias?: string } = { url: originalUrl };
      if (customAlias.trim()) {
      ...
      if (!token) {
        ...
        setStats({ totalClicks });
      } else { // LOGGED IN - Prepend new URL to state and update sessionStorage
      ...
      if (err.response?.status === 409) { // Conflict
        ...
        setError(err.response?.data?.error || 'Unable to shorten the URL.');
      }
    } finally {
      setLoading(false);
    }
  ...
  // HANDLE URL DELETION
  // ============================================================
  const handleDelete = async (shortIdToDelete: string) => {
